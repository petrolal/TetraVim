...
```bash
docker run --net=host -it --rm alpine:edge sh -c "apk add --no-cache curl bash && curl -fsSL https://raw.githubusercontent.com/petrolal/TetraVim/main/install.sh | bash && nvim"
```
